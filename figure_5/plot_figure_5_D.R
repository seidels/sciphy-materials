...
#Calculating the growth rate with a timeline matching our analysis, ignoring intermediate points between 7 and 10 days. 
...
# the growth rate r under exponential growth is rate r = (1/(t1-t0))*log(N1/N0), time intervals are 1 and 3 for the time bins used. 
rate <- c()
for(i in 0:22) { 
...
p_growth_OU_2 <- ggplot(growth_combined) +  
  ...
  geom_stepribbon(data=merle_et_al_bins,aes(x=t,ymin=low, ymax=high, fill=tree, col=tree), linetype="dotted", alpha = 0.1)+    
...
#Calculating the growth rate at the 1 day resolution using Merle et al. data.
