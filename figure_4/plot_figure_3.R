...
all_ltt_constant <- rbind(all_ltt_constant, cbind(ltt_MCC_constant,rep(length(trees_constant)+2,nrow(ltt_MCC_constant))))
...
all_ltt_constant$type <- c(rep("SciPhy constant",length(which(all_ltt_constant$number<=(length(trees_constant))))),rep("UPGMA origin",length(which(all_ltt_constant$number==(length(trees_constant)+1)))),rep("SciPhy MCC",length(which(all_ltt_constant$number==(length(trees_constant)+2)))))
...
###UPGMA + SCIPHY 
MCC_UPGMA_SCALED <- ape::read.nexus(file = "/Users/azwaans/typewriter_analysis/results/analysis_cell_culture_data/fixed_tree_analyses/fixed_trees/1000_UPGMA_medianPosteriorHeight_estimateBranchLengths_nodeReheight_combined_thinned1000000_MCC.tree")
ltt_mcc_upgma_scaled <- ltt.plot.coords(MCC_UPGMA_SCALED)   
all_ltt <- rbind(all_ltt, cbind(ltt_mcc_upgma_scaled,rep(length(trees)+3,nrow(ltt_mcc_upgma_scaled))))
...
all_ltt$type <- c(rep("SciPhy dynamic",length(which(all_ltt$number<=(length(trees))))),rep("UPGMA origin",length(which(all_ltt$number==(length(trees)+1)))),rep("SciPhy MCC",length(which(all_ltt$number==(length(trees)+2)))),rep("UPGMA - SciPhy",length(which(all_ltt$number==(length(trees)+3)))))
#all_ltt$type <- c(rep("SciPhy dynamic",length(which(all_ltt$number<=(length(trees))))),rep("UPGMA origin",length(which(all_ltt$number==(length(trees)+1)))),rep("SciPhy MCC",length(which(all_ltt$number==(length(trees)+2))),rep("UPGMA - SciPhy ",length(which(all_ltt$number==(length(trees)+3))))))


cols <- c("SciPhy constant" ="#56996E", "UPGMA origin" = "red","UPGMA root (constant)" = "#E07E5E","SciPhy MCC" = "black", "SciPhy dynamic" =  "#E1E1F7", "UPGMA" = "red","UPGMA root (13-epochs)" = "#E07E5E", "UPGMA - SciPhy" = "yellow")
alphas <- c("SciPhy constant" = 0.3, "UPGMA origin" = 1.0,"UPGMA root (constant)" = 1.0,"SciPhy MCC" = 1.0,"SciPhy dynamic" = 0.3, "UPGMA" = 1.0,"UPGMA root (13-epochs)" = 1.0, "UPGMA - SciPhy" = 0.3)
all_ltt$type = factor(all_ltt$type, levels=c("SciPhy constant", "SciPhy dynamic","UPGMA origin", "UPGMA - SciPhy", "SciPhy MCC"))
...
## Add the main analysis estimates
typewriter_file <- "UPGMA_reheight.log"
typewriter <- read.table(typewriter_file, header = T) %>% slice_tail(prop = 0.10)
bd_rates <- data.frame(growthRate = typewriter[,"birthRate"] - typewriter[,"deathRate"])
bd_rates$tree = "UPGMA - SciPhy"
bd_rates = melt(bd_rates, id.vars = "tree")
...
# Define order
growth_rates$tree = factor(growth_rates$tree, levels=c("Prior", "SciPhy posterior", "UPGMA origin", "UPGMA - SciPhy"))
#define colors
cols <- c("Prior" = "white","SciPhy posterior" = "#56996E", "UPGMA origin" = "red","UPGMA root" = "#E07E5E","SciPhy MCC" = "grey","UPGMA - SciPhy" = "yellow")
...
#load the log file with 2 change points in the estimated rates
log_file <- "~/typewriter_analysis/results/analysis_cell_culture_data/inference_results/clock_per_target_OU/combined_burnin10.log"
typewriter <- read.table(log_file, header = T)
typewriter_mcmc <- as.mcmc(typewriter)
growth <- typewriter_mcmc[,paste0("birthRate.",1:13)] - typewriter_mcmc[,paste0("deathRate.",1:13)]
growth_hpd = get_median_and_hpd(growth)
growth_hpd[14, ] =  growth_hpd[13, ]# add last timepoint, st for time - 11 the same value remains
growth_hpd$t = timeline_changes
growth_hpd$tree = "SciPhy dynamic"
growth_combined = rbind(growth_combined, growth_hpd)


#load the log file with 2 change points in the estimated rates
log_file <- "UPGMA_reheight_OU.log"
typewriter <- read.table(log_file, header = T)
typewriter_mcmc <- as.mcmc(typewriter)
growth <- typewriter_mcmc[,paste0("birthRate.",1:13)] - typewriter_mcmc[,paste0("deathRate.",1:13)]
growth_hpd = get_median_and_hpd(growth)
growth_hpd[14, ] =  growth_hpd[13, ]# add last timepoint, st for time - 11 the same value remains
growth_hpd$t = timeline_changes
growth_hpd$tree = "UPGMA - SciPhy"
growth_combined = rbind(growth_combined, growth_hpd)

#set colors and fills manually
cols <- c("SciPhy dynamic" = "#A4A4F2", "UPGMA origin" = "red", "UPGMA - SciPhy" = "yellow")
cols_fill<- c("SciPhy dynamic" = "#A4A4F2", "UPGMA origin" = "red", "UPGMA - SciPhy" = "yellow")
p_growth_OU = ggplot(growth_combined, aes(x=t, y=median, col=tree)) +
  geom_step(size=1)+
  geom_stepribbon(aes(ymin = hpd_low, ymax=hpd_up, fill=tree, col=tree), linetype="dotted", alpha = 0.1)+
...
row_3 <- cowplot::plot_grid(p_growth+ theme(legend.position = "none",panel.grid = element_blank(),axis.ticks.x = element_blank(),panel.border = element_blank(),axis.text.x = element_blank(),axis.line.x = element_line("black"),axis.line.y = element_line("black")) ,p_growth_OU + theme(legend.position = "none"),ncol=2,nrow=1, rel_widths = c(1.0,1.0),axis = "l")

ggsave(paste0(pic_dir,"row_3.pdf"),row_3, width = 14.28, height = 7.14, units = "cm", dpi = 300)
