...
##
##
## Script name: create_supp_fig_17
##
...
# set figure settings
text_size=10
figure_path = "plots/supp_fig_17.pdf"

# load inference logs
log_seq_data <- "../figure_3/inference_output/combined.log"
dat_seq_data <- read.table(log_seq_data, header = T)
dat_seq_data$growth_rate = dat_seq_data$birthRate - dat_seq_data$deathRate
dat_seq_data$analysis = "with sequence data"
...
dat_no_seq_data$analysis = "w/o sequence data"

dat_combined = rbind(dat_seq_data[, c("growth_rate", "analysis")], dat_no_seq_data[, c("growth_rate", "analysis")])

growth_plots = 
  ggplot(dat=subsampled, aes(x=analysis, y=growth_rate))+
  geom_violin()+
  ylim(0.4, 0.87)+
  labs(y = expression("Posterior growth rate [" * d^-1 * "]"),x="")+
  theme_bw()

growth_plots

ggsave(figure_path, plot = growth_plots,height = 7.14, units = "cm", dpi = 300)
